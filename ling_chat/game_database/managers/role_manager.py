...
class RoleManager:
    ...
    def sync_roles_from_folder(game_data_path: Path) -> List[int]:
        ...
        with Session(engine, expire_on_commit=False) as session:
            ...
            db_roles_map = {
                role.resource_folder: role for role in results if role.resource_folder
            }
    ...
    def get_role_by_script_keys(
        script_key: str, script_role_key: str
    ) -> Optional[Role]:
        """通过 script_key 查找角色 (可能返回多个中的第一个)"""
        with Session(engine) as session:
            stmt = select(Role).where(
                Role.script_key == script_key, Role.script_role_key == script_role_key
            )
            return session.exec(stmt).first()
    ...
    def get_role_settings_by_id(role_id: int) -> Optional[CharacterSettings]:
        ...
        if (
            not (path / "settings.yml").exists()
            and not (path / "settings.txt").exists()
        ):
