...
class UserActivityMonitor(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.listener_m = mouse.Listener(
    ...
    def stop(self):
        self.running = False
        if hasattr(self, "listener_k"):
            self.listener_k.stop()
        if hasattr(self, "listener_m"):
    ...
    def _on_move(self, x, y):
        ...
        with self.lock:
            if self.last_mouse_pos:
                self.mouse_distance += abs(x - self.last_mouse_pos[0]) + abs(
    ...
    def _analyze(self):
        ...
        with self.lock:
            while (
                self.event_queue and now - self.event_queue[0][0] > self.window_seconds
            ):
