    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                response = await self._process_sentence_and_prepare_response(
                ...
                logger.error(
                    f"Error in consumer {self.consumer_id}: {e}", exc_info=True
                )
                # 使用 traceback 模块获取详细的错误信息
                traceback.print_exc()
                self.sentence_queue.task_done()

    async def _process_sentence_and_prepare_response(
        self, sentence: str, user_message: str, is_final: bool
        ...
        response = ResponseFactory.create_reply(
            sentence_segments[0], user_message, is_final
        )
        ai_line = LineBase(
