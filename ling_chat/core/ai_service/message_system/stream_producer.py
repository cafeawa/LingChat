    ...
    async def run(self) -> str:
        ...
        try:
            async for chunk in ai_response_stream:
                ...
                if (
                    len(realtime_display_buffer) >= 3  # 每3个字符显示一次
                    or current_time - last_display_time > 0.1  # 或者每100毫秒
                ...
                while "【" in buffer:
                    ...
                    if sentence and "】" in buffer:
                        ...
                        if next_start != -1:
                            sentence += buffer[
                                :next_start
                            ]  # buffer 删除情绪后面跟随的句子和动作等信息"你好呀（摇尾巴）"
                        ...
                        self.publish_events[current_index] = (
                            asyncio.Event()
                        )  # 为这个索引创建一个事件
                        await self.sentence_queue.put(
                            (sentence, current_index, False)
                        )  # is_final=False
                        sentence_index += 1
