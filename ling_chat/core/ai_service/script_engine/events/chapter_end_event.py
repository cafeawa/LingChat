...
class ChapterEndEvent(BaseEvent):
        ...
        for opt in options:
            ...
            if condition:
                if ScriptFunction.evaluate(condition, vars_dict):
                    logger.info(
                        f"条件 '{condition}' 满足，选择分支: {opt.get('name', opt.get('next'))}"
                    )
    ...
    async def _handle_ai_judged(self) -> str:
        ...
        try:
            ...
            matched_next = ScriptFunction.match_ai_response_options(
                ai_response, options
            )
            if matched_next:
                return matched_next
            ...
            logger.warning(
                f"AI 结果 '{ai_response}' 未匹配任何选项，且无默认，返回 end"
            )
            return "end"
        ...
        if self.game_status.line_list:
            for line in reversed(self.game_status.line_list[-limit:]):
                if line.attribute in (LineAttribute.USER, LineAttribute.ASSISTANT):
                    speaker = line.display_name or (
                        "玩家" if line.attribute == LineAttribute.USER else "角色"
                    )
                    lines.append(f"{speaker}: {line.content}")
        return list(reversed(lines))

    def _build_ai_prompt(
        self, prompt: str, dialogue: List[str], options: List[dict]
    ) -> str:
    ...
    async def _call_llm(self, prompt: str) -> str:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None, ai_service.llm_model.process_message, messages
            )
